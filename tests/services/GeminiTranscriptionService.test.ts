...
global.FileReader = jest.fn().mockImplementation(function (this: any) {
  this.readAsDataURL = jest.fn(function (this: any, blob: Blob) {
...
describe("GeminiTranscriptionService", () => {
  ...
  const createServiceWithMock = (
    ...
    (service as any).connector = new MockGeminiConnector(config.apiKey, mockResponse);
    return service;
  };
  ...
  describe("constructor", () => {
    ...
    it("should accept optional model in configuration", () => {
      ...
      expect(service).toBeInstanceOf(GeminiTranscriptionService);
      expect((service as any).config.model).toBe(TEST_CUSTOM_MODEL);
    });
    ...
    it("should accept optional language in configuration", () => {
      ...
      expect(service).toBeInstanceOf(GeminiTranscriptionService);
      expect((service as any).config.language).toBe("en");
    });
    ...
    it("should accept optional customPrompt in configuration", () => {
      ...
      expect(service).toBeInstanceOf(GeminiTranscriptionService);
      expect((service as any).config.customPrompt).toBe(TEST_CUSTOM_PROMPT);
    });
  });
  ...
  describe("transcribe", () => {
    ...
    it("should handle different mime types", async () => {
      ...
      for (const mimeType of mimeTypes) {
        ...
        const result = await service.transcribe(audioBlob, TEST_FILE_NAME, mimeType);
    ...
    it("should ignore fileName parameter (per interface contract)", async () => {
      ...
      const result = await service.transcribe(audioBlob, ignoredFileName, TEST_MIME_TYPE);
    ...
    it("should handle blob to base64 conversion errors", async () => {
      ...
      const mockFileReaderError = jest.fn().mockImplementation(function (this: any) {
  ...
  describe("blobToBase64 conversion", () => {
    ...
    it("should handle different blob sizes", async () => {
      ...
      const service1 = createServiceWithMock(config, createSuccessResponse(TEST_TRANSCRIPTION));
      const smallBlob = new Blob(["small"], { type: TEST_MIME_TYPE });
      const result1 = await service1.transcribe(smallBlob, "small.mp3", TEST_MIME_TYPE);
      expect(result1).toBe(TEST_TRANSCRIPTION);

      // Test with larger blob
      const service2 = createServiceWithMock(config, createSuccessResponse(TEST_TRANSCRIPTION));
      const largeData = "x".repeat(10000);
      const largeBlob = new Blob([largeData], { type: TEST_MIME_TYPE });
      const result2 = await service2.transcribe(largeBlob, "large.mp3", TEST_MIME_TYPE);
      expect(result2).toBe(TEST_TRANSCRIPTION);
    });
  });
});
