...
import { SERVICES, getAvailableProviders, getProviderOptions } from './ServiceRegistry';
...
export class TranscriptionServiceFactory {
	 ...
	 */
	static create(provider: string, config: TranscriptionServiceConfig): ITranscriptionService {
		const registration = SERVICES[provider];
		
		if (!registration) {
			const availableProviders = getAvailableProviders().join(', ');
			throw new Error(`Unknown transcription provider: ${provider}. Available providers: ${availableProviders}`);
		}
